...
int main()
{
    ...
    mysql_real_connect(&mysql,"127.0.0.1","root","200713","jieni",0,NULL,0);//将mysql库的信息存入到mysql句柄
   ...
   // ret = mysql_query(&mysql,"select *from know where name = \"lt\"");//query select
    char buff[100];
    char lt[20] = "lt";
    sprintf(buff, "select *from know where name = \"%s\"", lt);//连接两个字符串
    ...
    if(!ret)
    {
      ...
      //  num_fields=mysql_num_fields(result);    //返回列数
